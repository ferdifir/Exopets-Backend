...
exports.createStore = (uid, store_name, address, description, callback) => {
    const query = 'INSERT INTO stores (user_id, nama_toko, alamat, description) VALUES (?, ?, ?, ?)';
...
exports.getStoreByUid = (uid, callback) => {
    const query = 'SELECT * FROM stores JOIN users ON stores.user_id = users.uid WHERE stores.user_id = ?';
    const values = [uid];
